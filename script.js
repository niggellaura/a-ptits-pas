...
// Contact 100 % statique : aucune donnée n'est envoyée à un service tiers.
document.querySelectorAll('[data-contact-form]').forEach(form => form.addEventListener('submit', e => {
  e.preventDefault();
  if (!form.reportValidity()) return;
  const first = form.querySelector('[name="prenom"]')?.value.trim() || '';
  const last = form.querySelector('[name="nom"]')?.value.trim() || '';
  const email = form.querySelector('[name="email"]')?.value.trim() || '';
  const subject = form.querySelector('[name="sujet"]')?.value || 'Contact via le site';
  const body = form.querySelector('[name="message"]')?.value.trim() || '';
  const fullBody = `Prénom / Nom : ${first} ${last}\nE-mail : ${email}\n\n${body}`;
  const message = form.querySelector('.form-message');
  if (message) { message.hidden = false; message.textContent = 'Votre application de messagerie va s’ouvrir. Aucun message n’est envoyé directement par le site.'; }
  window.location.href = `mailto:assoaptitspas@gmail.com?subject=${encodeURIComponent(subject)}&body=${encodeURIComponent(fullBody)}`;
}));
